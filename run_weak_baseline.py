...
epses = [0.01, 0.05, 0.1, 0.2, 0.4, 0.6]
...
for eps in epses:
    for baseline in baselines:
        ...
        cmds.append(cmd)
cmds.append("MP_NUM_THREADS=4 CUDA_VISIBLE_DEVICES=%d  python weakly-main.py --baseline %s --eps %f"%(np.random.choice(choice_of_GPU),'ADMM_weak_gc',0))
